<template>
  ...
  <div class="bg-green-100 py-14">
    ...
    <h1 class="mt-8 text-5xl font-bold text-center text-green-600">Nhanh chóng & Chính xác.</h1>

    <!-- Box -->
    <div :class="boxStyle">
      <!-- box-1 -->
      <div v-for="(item,index) in contentData" :key="index" :class="cardStyle">
        <div class="w-sm">
          <img class="w-64" :src="item.fImageLink" alt />
          <div class="mt-4 text-center text-green-600">
            <h1 class="text-xl font-bold">{{ item.title }}</h1>
            <p class="mt-4 text-gray-600">{{ item.content }}</p>
            <button :class="buttonStyle">{{ item.button }}</button>
          </div>
        </div>
      </div>
    </div>
  ...
  data() {
    return {
      ...
      contentData: [{
        title: 'A',
        content: 'contentA',
        fImageLink: "https://images01.nicepage.com/c461c07a441a5d220e8feb1a/a17abde8d83650a582a28432/users-with-speech-bubbles-vector_53876-82250.jpg",
        button: 'ButtonName'
      }, {
        title: 'B',
        content: 'contentB',
        fImageLink: "https://images01.nicepage.com/c461c07a441a5d220e8feb1a/3b242447f922540fbe750cab/fdf.jpg",
        button: 'ButtonName'
      }, {
        title: 'C',
        content: 'contentC',
        fImageLink: "https://images01.nicepage.com/c461c07a441a5d220e8feb1a/8cc47b39e719570b996d9879/dsds.jpg",
        button: 'ButtonName'
      }]
    }
  }, mounted() {
    ...
    testFetchDb() {
      get(child(ref(firebaseDb), `data`)).then((snapshot) => {
        if (snapshot.exists()) {
